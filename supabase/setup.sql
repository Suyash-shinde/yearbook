...
-- ----------------------------------------------------------------------------
-- Submission gate. Submissions are open indefinitely — no deadline. To seal the
-- yearbook later, add a `raise exception` here and flip isLocked() in
-- src/config.js to match. Called by every mutating function below.
-- ----------------------------------------------------------------------------
create or replace function public.assert_open() returns void
language plpgsql as $$
begin
  -- open: nothing to assert
end;
$$;
